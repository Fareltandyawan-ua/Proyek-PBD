...
<div class="pemilik-container">
    <form method="post">
        ...
        <select name="idreservasi_dokter" required>
            ...
            <?php foreach ($reservasi['data'] as $r) {
                $selected = ($edit_data && $edit_data['idreservasi_dokter'] == $r['idreservasi_dokter']) ? 'selected' : '';
                echo '<option value="' . $r['idreservasi_dokter'] . '" ' . $selected . '>No: ' . $r['no_urut'] . ' | Pet: ' . $r['idpet'] . '</option>';
            } ?>
        </select>
        <input type="text" name="anamnesa" placeholder="Anamnesa" value="<?= $edit_data ? htmlspecialchars($edit_data['anamnesa']) : '' ?>" required>
        <input type="text" name="temuan_klinis" placeholder="Temuan Klinis" value="<?= $edit_data ? htmlspecialchars($edit_data['temuan_klinis']) : '' ?>" required>
        <input type="text" name="diagnosa" placeholder="Diagnosa" value="<?= $edit_data ? htmlspecialchars($edit_data['diagnosa']) : '' ?>" required>
        <input type="number" name="dokter_pemeriksa" placeholder="ID Dokter" value="<?= $edit_data ? htmlspecialchars($edit_data['dokter_pemeriksa']) : '' ?>" required>
    ...
    <table>
        <tr><th>ID</th><th>Reservasi</th><th>Anamnesa</th><th>Temuan Klinis</th><th>Diagnosa</th><th>Dokter Pemeriksa</th><th>Aksi</th></tr>
        <?php foreach ($data as $row) { ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="?delete=<?= $row['idrekam_medis'] ?>" class="button" style="background:#d90429;" onclick="return confirm('Hapus data ini?')">Delete</a>
                    <a href="detail_rekam_medis.php?idrekam_medis=<?= $row['idrekam_medis'] ?>" class="button" style="background:#43aa8b;">Lihat Detail</a>
                </td>
            </tr>
        <?php } ?>
    </table>
