...
class DetailRekamMedis {
    ...
    public function getByRekamMedis($idrekam_medis) {
        $stmt = $this->db->prepare("SELECT d.*, k.kode, k.deskripsi_tindakan_terapi 
            FROM detail_rekam_medis d
            JOIN kode_tindakan_terapi k ON d.idkode_tindakan_terapi = k.idkode_tindakan_terapi
            WHERE d.idrekam_medis = ?");
        $stmt->bind_param("i", $idrekam_medis);
        $stmt->execute();
        $result = $stmt->get_result();
        $stmt->close();
