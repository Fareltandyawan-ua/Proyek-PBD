...
$pemiliks = [];
$pemilik_result = $dbconn->query("SELECT idpemilik, user.nama, no_wa, alamat FROM pemilik
                                        JOIN user ON user.iduser = pemilik.iduser");
            ...
            <select name="idpemilik" required>
                ...
                <?php foreach ($pemiliks as $pemilik): ?>
                    <option value="<?= $pemilik['idpemilik'] ?>">
                        <?= $pemilik['idpemilik'] ?> - <?= htmlspecialchars($pemilik['nama']) ?>
                        (<?= htmlspecialchars($pemilik['alamat']) ?>)
                    </option>
                <?php endforeach; ?>
            </select>
