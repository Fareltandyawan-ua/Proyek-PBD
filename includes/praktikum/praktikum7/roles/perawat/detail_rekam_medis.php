...
if (isset($_POST['tambah'])) {
    $detail->create($_POST);
    header('Location: detail_rekam_medis.php?idrekam_medis=' . $_POST['idrekam_medis']);
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    $id = isset($_GET['idrekam_medis']) ? intval($_GET['idrekam_medis']) : '';
    header('Location: detail_rekam_medis.php?idrekam_medis=' . $id);
    exit;
}
$edit_data = null;
if (isset($_GET['edit'])) {
...
<body>
    ...
    <div class="pemilik-container">
        <form method="post">
            <?php if ($edit_data) { ?>
                <input type="hidden" name="iddetail_rekam_medis"
                    value="<?= htmlspecialchars($edit_data['iddetail_rekam_medis']) ?>">
            <?php } ?>
            ...
            <input type="text" name="detail" placeholder="Detail"
                value="<?= $edit_data ? htmlspecialchars($edit_data['detail']) : '' ?>" required>
        ...
        <?php if ($idrekam_medis) { ?>
            <table>
                ...
                <?php foreach ($data as $row) { ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['kode']) ?> - <?= htmlspecialchars($row['deskripsi_tindakan_terapi']) ?>
                        </td>
                        <td><?= htmlspecialchars($row['detail']) ?></td>
                        <td>
                            <a href="?edit=<?= $row['iddetail_rekam_medis'] ?>&idrekam_medis=<?= $idrekam_medis ?>"
                                class="button">Edit</a>
                            <a href="?delete=<?= $row['iddetail_rekam_medis'] ?>&idrekam_medis=<?= $idrekam_medis ?>"
                                class="button" style="background:#d90429;"
            ...
            <br>
            <a href="rekam_medis.php" class="button" style="background:#4FC3F7;margin-top:16px;"><< Kembali </a>
        <?php } ?>
    </div>
</body>
