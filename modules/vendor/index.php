...
try {
  if ($statusFilter === '1') {
    // status aktif
    $vendorList = $db->fetchAll("SELECT * FROM v_vendor_aktif ORDER BY idvendor ASC");
  } elseif ($statusFilter === '0') {
    // status tidak aktif
    $vendorList = $db->fetchAll("SELECT * FROM v_vendor WHERE status = 0 ORDER BY idvendor ASC");
  } else {
    // Semua status
    $vendorList = $db->fetchAll("SELECT * FROM v_vendor ORDER BY idvendor ASC");
  }
} catch (Exception $e) {
        ...
        <table class="table table-hover align-middle">
          ...
          <tbody>
            <?php if (empty($vendorList)): ?>
              <tr>
                <td colspan="5" class="text-center text-muted py-4"><i class="fas fa-inbox fa-2x mb-2 d-block"></i>Tidak
                  ada data vendor.</td>
              </tr>
            <?php else:
              foreach ($vendorList as $i => $v): ?>
                <tr>
                  <td><?= $i + 1 ?></td>
                  ...
                  <td class="text-center">
                    <a href="edit.php?id=<?= $v['idvendor'] ?>" class="btn btn-warning btn-sm"><i
